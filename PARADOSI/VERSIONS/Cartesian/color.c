...
int main(int argc, char *argv[]) {
	...
	int *displs; /* The starting point of everyone's data in the global array, in block extents ( colsNumber/sqrt_comm_sz * 3 *  MPI_UNSIGNED_CHARs )  */
	...
	int dim[2] = {sqrt_comm_sz, sqrt_comm_sz};
	int period[2] = {0,0};
	int reorder = 0;
	...
	for(i=0; i<filterApplications; i++){
		...
		MPI_Isend(myArray[0],1,row,myNeighbors.top.rank,0,new_comm,&myNeighbors.top.sendRequest);

		/* Send my leftUpCorn to my leftup process */
		MPI_Isend(&myArray[0][0],3,MPI_UNSIGNED_CHAR,myNeighbors.leftUp.rank,0,new_comm,&myNeighbors.leftUp.sendRequest);

		/* Send my leftCol to my left process */
		MPI_Isend(&myArray[0][0],1,column,myNeighbors.left.rank,0,new_comm,&myNeighbors.left.sendRequest);

		/* Send my bottomRow to my bottom process */
		MPI_Isend(myArray[(rowsNumber/sqrt_comm_sz)-1],1,row,myNeighbors.bottom.rank,0,new_comm,&myNeighbors.bottom.sendRequest);

		/* Send my leftDownCorn to my leftdown process */
		MPI_Isend(&myArray[(rowsNumber/sqrt_comm_sz)-1][0],3,MPI_UNSIGNED_CHAR,myNeighbors.leftDown.rank,0,new_comm,&myNeighbors.leftDown.sendRequest);

		/* Send my rightUpCorn to my rightup process */
		MPI_Isend(&myArray[0][3*(colsNumber/sqrt_comm_sz)-3],3,MPI_UNSIGNED_CHAR,myNeighbors.rightUp.rank,0,new_comm,&myNeighbors.rightUp.sendRequest);

		/* Send my rightCol to my right process */
		MPI_Isend(&myArray[0][3*(colsNumber/sqrt_comm_sz)-3],1,column,myNeighbors.right.rank,0,new_comm,&myNeighbors.right.sendRequest);

		/* Send my rightDownCorn to my rightdown process */
		MPI_Isend(&myArray[(rowsNumber/sqrt_comm_sz)-1][3*(colsNumber/sqrt_comm_sz)-3],3,MPI_UNSIGNED_CHAR,myNeighbors.rightDown.rank,0,new_comm,&myNeighbors.rightDown.sendRequest);


		/* Recieve rows,columns and corners */

		/* Recieve topRow from my top process */
		MPI_Irecv(topRow,1,row,myNeighbors.top.rank,0,new_comm, &myNeighbors.top.recieveRequest);

		/* Recieve leftUpCorn from my leftup process */
		MPI_Irecv(leftUpCorn,3,MPI_UNSIGNED_CHAR,myNeighbors.leftUp.rank,0,new_comm, &myNeighbors.leftUp.recieveRequest);

		/* Recieve leftCol from my left process */
		MPI_Irecv(leftCol,3*(rowsNumber/sqrt_comm_sz),MPI_UNSIGNED_CHAR,myNeighbors.left.rank,0,new_comm, &myNeighbors.left.recieveRequest);

		/* Recieve bottomRow from my bottom process */
		MPI_Irecv(bottomRow,1,row,myNeighbors.bottom.rank,0,new_comm, &myNeighbors.bottom.recieveRequest);

		/* Recieve leftDownCorn from my leftdown process */
		MPI_Irecv(leftDownCorn,3,MPI_UNSIGNED_CHAR,myNeighbors.leftDown.rank,0,new_comm, &myNeighbors.leftDown.recieveRequest);

		/* Recieve rightUpCorn from my rightup rocess */
		MPI_Irecv(rightUpCorn,3,MPI_UNSIGNED_CHAR,myNeighbors.rightUp.rank,0,new_comm, &myNeighbors.rightUp.recieveRequest);

		/* Recieve rightCol from my right process */
		MPI_Irecv(rightCol,3*(rowsNumber/sqrt_comm_sz),MPI_UNSIGNED_CHAR,myNeighbors.right.rank,0,new_comm, &myNeighbors.right.recieveRequest);

		/* Recieve rightDownCorn from my rightdown process */
		MPI_Irecv(rightDownCorn,3,MPI_UNSIGNED_CHAR,myNeighbors.rightDown.rank,0,new_comm, &myNeighbors.rightDown.recieveRequest);
		...
		for(int i=1; i<(rowsNumber/sqrt_comm_sz)-1; i++){
			for(int j=3; j<3*((colsNumber/sqrt_comm_sz)-1); j = j+3 ){
				for(int c=0; c<=2; c++ ){ /* For 3 colors j,j+1,j+2 */
					myFinalArray[i][j+c] = myArray[i-1][j-3+c]*((double)1/16)		/* leftUpCorn */
										 + myArray[i][j-3+c]*((double)2/16)			/* left */
										 + myArray[i+1][j-3+c]*((double)1/16)		/* leftDownCorn */
										 + myArray[i-1][j+c]*((double)2/16)			/* up */
										 + myArray[i][j+c]*((double)4/16)			/* itself */
										 + myArray[i+1][j+c]*((double)2/16)			/* down */
										 + myArray[i-1][j+3+c]*((double)1/16)		/* rightUpCorn */
										 + myArray[i][j+3+c]*((double)2/16)			/* right */
										 + myArray[i+1][j+3+c]*((double)1/16);		/* rightDownCorn*/
				}
			}
		}
	...
	MPI_Gatherv(*myArray,(rowsNumber/sqrt_comm_sz)*3*(colsNumber/sqrt_comm_sz),MPI_UNSIGNED_CHAR,*final2D,counts,displs,resizedtype,0,new_comm );
