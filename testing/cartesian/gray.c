...
int main(int argc, char *argv[]) {
	...
	for(i=0; i<filterApplications; i++){
		...
		MPI_Isend(&myArray[0][0],1,MPI_UNSIGNED_CHAR,myNeighbors.leftUp.rank,0,new_comm,&myNeighbors.leftUp.sendRequest);
		...
		MPI_Isend(&myArray[(rowsNumber/sqrt_comm_sz)-1][0],1,MPI_UNSIGNED_CHAR,myNeighbors.leftDown.rank,0,new_comm,&myNeighbors.leftDown.sendRequest);

		/* Send my rightUpCorn to my rightup process */
		MPI_Isend(&myArray[0][(colsNumber/sqrt_comm_sz)-1],1,MPI_UNSIGNED_CHAR,myNeighbors.rightUp.rank,0,new_comm,&myNeighbors.rightUp.sendRequest);

		/* Send my rightCol to my right process */
		MPI_Isend(&myArray[0][(colsNumber/sqrt_comm_sz)-1],1,column,myNeighbors.right.rank,0,new_comm,&myNeighbors.right.sendRequest);

		/* Send my rightDownCorn to my rightdown process */
		MPI_Isend(&myArray[(rowsNumber/sqrt_comm_sz)-1][(colsNumber/sqrt_comm_sz)-1],1,MPI_UNSIGNED_CHAR,myNeighbors.rightDown.rank,0,new_comm,&myNeighbors.rightDown.sendRequest);
		...
		MPI_Irecv(&leftUpCorn,1,MPI_UNSIGNED_CHAR,myNeighbors.leftUp.rank,0,new_comm, &myNeighbors.leftUp.recieveRequest);

		/* Recieve leftCol from my left process */
		MPI_Irecv(leftCol,(rowsNumber/sqrt_comm_sz),MPI_UNSIGNED_CHAR,myNeighbors.left.rank,0,new_comm, &myNeighbors.left.recieveRequest);
		...
		MPI_Irecv(&leftDownCorn,1,MPI_UNSIGNED_CHAR,myNeighbors.leftDown.rank,0,new_comm, &myNeighbors.leftDown.recieveRequest);

		/* Recieve rightUpCorn from my rightup rocess */
		MPI_Irecv(&rightUpCorn,1,MPI_UNSIGNED_CHAR,myNeighbors.rightUp.rank,0,new_comm, &myNeighbors.rightUp.recieveRequest);

		/* Recieve rightCol from my right process */
		MPI_Irecv(rightCol,(rowsNumber/sqrt_comm_sz),MPI_UNSIGNED_CHAR,myNeighbors.right.rank,0,new_comm, &myNeighbors.right.recieveRequest);

		/* Recieve rightDownCorn from my rightdown process */
		MPI_Irecv(&rightDownCorn,1,MPI_UNSIGNED_CHAR,myNeighbors.rightDown.rank,0,new_comm, &myNeighbors.rightDown.recieveRequest);
	...
	MPI_Gatherv(*myArray,(rowsNumber/sqrt_comm_sz)*(colsNumber/sqrt_comm_sz),MPI_UNSIGNED_CHAR,*final2D,counts,displs,resizedtype,0,new_comm );
